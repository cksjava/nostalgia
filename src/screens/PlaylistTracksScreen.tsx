// src/screens/PlaylistTracksScreen.tsx
...
import { playlistTracks, type PlaylistTrack } from "../data/playlistTracks";
...
export default function PlaylistTracksScreen() {
  ...
  // If route invalid, go back to playlists
  useEffect(() => {
  ...
  const startPlayback = (trackNo?: number, shuffle = false) => {
    console.log("Playlist playback:", { playlistId, trackNo, shuffle });
    navigate("/", {
      state: {
        fromSource: "playlist",
        playlistId,
      ...
      <div className="min-h-screen w-full backdrop-blur-2xl">
        ...
        <header className="mx-auto flex w-full max-w-md items-center justify-between px-4 pt-6">
          ...
          <div className="flex min-w-0 items-center gap-2 text-white/70">
            <FontAwesomeIcon icon={faListUl} className="shrink-0 text-white/60" />
            <span className="truncate text-sm tracking-widest">{playlist.name}</span>
          </div>
